...
sealed class DataMarketOracle : DataMarketInterface
{
    private DBOper<OracleCreator> db = new DBOper<OracleCreator>();
        ...
        get
        {
            if (this.tableDataSet == null)
            {
                try
                {
                    ...
                    // 取得数据库中列信息，表信息，外键信息
                    string sql = RegistFields.instance.ColumnsInfoSQL + "\r\n" + RegistFields.instance.TableSQL + "\r\n " + RegistFields.instance.FKSQL;
                    //sql = string.Format(sql, dataBase);
                    this.tableDataSet = new DataSet();
                    DataTable dt = db.ExecuteDataTable(RegistFields.instance.ColumnsInfoSQL, null, CommandType.Text);
                    dt.TableName = "ColumnInfo";
                    this.tableDataSet.Tables.Add(dt);

                    dt = db.ExecuteDataTable(RegistFields.instance.TableSQL, null, CommandType.Text);
                    dt.TableName = "TableInfo";
                    this.tableDataSet.Tables.Add(dt);

                    dt = db.ExecuteDataTable(RegistFields.instance.FKSQL, null, CommandType.Text);
                    dt.TableName = "FKInfo";
                    this.tableDataSet.Tables.Add(dt);
                    /*
                    string[] names = { "ColumnInfo", "TableInfo", "FKInfo" };
                    this.tableDataSet = DBOperOracle.ExecuteDataSet(sql, names);*/

                    dt = this.tableDataSet.Tables["TableInfo"];
                    ...
                    {
                        r["ID"] = index;
                        if (r["script"] == DBNull.Value)
                        {
                            r["script"] = r["TableName"];
                        }
                        index++;
                    }
                }
    ...
    public DataView GetTableFK(int tableId)
    {
        ...
        DataView dv = new DataView(dt);
        string filter = "table_name = " + GetTableName(tableId);
        dv.RowFilter = filter;
        return dv;
    }
    ...
    public bool IsFKForColumn(string tableName, string columnName)
    {
        DataTable dt = this.TableDataSet.Tables["FKInfo"];
        DataView dv = new DataView(dt);
        dv.RowFilter = " table_name = '" + tableName + "' AND ColumnName='" + columnName + "'";
        if (dv.Count < 1)
        {
            return false;
        }
    ...
    {
        dt.FKName = Convert.ToString(drv["constraint_name"]);
        dt.TableName = Convert.ToString(drv["table_name"]);
